...
void worker(void *word)
{
	printf("Thread from threadpool prints: %s\n", (char *)word);
}
...
int main(int argc, char **argv) {
	...
	int *vals;
	size_t i;
	char paragraph[] = "Hi My Name is Santosh. I stay in Bangalore.";
	char *word;
	bool status = false;

	unsigned int num_threads = *argv[1];
	...
	printf("Parent Thread prints: %s", word);

	while(word != NULL) {
		word = strtok(NULL, " ");
		status = tpool_add_work(tm, worker, (void *)word);
	}
